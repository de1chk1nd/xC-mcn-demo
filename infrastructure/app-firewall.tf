...
#######################################
# Troubleshooting: Extended WAF Policy
# This policy includes all advanced settings that should work but require
# a manual save in xC Console to propagate. Created for debugging purposes.
# TODO: Open support case if this does not activate on initial deploy.
#######################################
resource "volterra_app_firewall" "troubleshooting-waf" {
  name      = "${local.setup-init.student.name}-troubleshooting"
  namespace = local.setup-init.xC.namespace
